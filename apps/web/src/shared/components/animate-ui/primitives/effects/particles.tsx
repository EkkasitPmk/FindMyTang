...
} from "@/shared/lib/hooks/use-is-in-view";
...
function ParticlesEffect({
  ...
  let alignPct = "50%";
  if (align === "start") alignPct = "0%";
  if (align === "end") alignPct = "100%";

  let top = `calc(${alignPct} + ${alignOffset}px)`;
  if (isVertical) {
    top =
      side === "top"
  ...
  let left = `calc(${alignPct} + ${alignOffset}px)`;
  if (!isVertical) {
    left =
      side === "left"
        ? `calc(0% - ${sideOffset}px)`
