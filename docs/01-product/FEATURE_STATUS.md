...
- [x] แก้ไข TypeScript Type Error ใน Asset Hook (`apps/web/src/features/assets/hooks/assets.hook.ts`) โดยกำหนดค่าเริ่มต้น fallback ให้กับฟิลด์ `balance` (`?? 0`) เมื่อสร้างข้อมูลจำลองในโหมด Guest
...
- [x] **ลบฟีเจอร์ Currency ออกจากระบบทั้งหมด:** ดำเนินการลบฟิลด์และตัวเลือกสกุลเงิน (Currency) ออกจากทั้งในระดับฐานข้อมูล (Prisma schemas), REST API (DTOs, Service logic, Controller mappings), Frontend Web UI (AssetForm, Containers, Types, Zod Schema, Guest Hooks, More Page settings) และเอกสารโครงการทั้งหมดอย่างหมดจด
...
- [x] **แก้ไขบั๊กฟอร์มสร้างสินทรัพย์ (AssetForm) และความเข้ากันได้ทาง Accessibility (a11y):** แก้ไขปัญหาช่อง Specify Type (สำหรับประเภทสินทรัพย์แบบ OTHER) ดึงค่า default เป็น `"CASH"` โดยการแยกฟิลด์ Specify Type ออกเป็นฟิลด์ใหม่ชื่อ `"otherType"` ใน Zod Schema และ React Hook Form แยกจากฟิลด์ `"type"` อย่างเด็ดขาด และปรับปรุงให้ฟังก์ชัน `handleSelect` เรียก `setValue("type", type)` เพื่ออัปเดตสถานะของฟอร์มหลักให้ถูกต้องเมื่อเปลี่ยนตัวเลือกจาก Dropdown พร้อมกับปรับปรุงตัวเลือกประเภทสินทรัพย์ใน Dropdown โดยเปลี่ยนจากการผูกเหตุการณ์ `onClick` ไว้บนแท็ก `<li>` (ซึ่งไม่ใช่ interactive element ตามข้อกำหนด accessibility) ไปใช้ปุ่ม `<button type="button">` ที่ถูกต้องตามหลัก Semantic HTML และลบโค้ด/พร็อพส์ที่ไม่ได้ใช้งานออก
- [x] **ปรับปรุงตำแหน่งปุ่ม Submit ในฟอร์มสร้างสินทรัพย์ (AssetForm) สำหรับ Mobile:** ย้ายปุ่ม Create Asset ให้ปักหมุดอยู่ที่ด้านล่างสุดของหน้าจอมือถือ (`fixed bottom-0`) เพื่อให้กดง่ายและเข้าถึงสะดวก พร้อมปรับปรุง Container หลักให้มี padding-bottom (`pb-28`) ป้องกันเนื้อหาด้านล่างสุดและปุ่มทับซ้อนกัน และตั้งค่า `z-index` ให้เมนู Dropdown เลือกประเภทสินทรัพย์ (`z-20`) ลอยอยู่เหนือปุ่ม submit เสมอเมื่อเปิดใช้งาน

### 5. การปรับปรุงระบบโครงสร้างพื้นฐาน (Infrastructure & Database Upgrade)
- [x] **อัปเกรด Prisma จาก v6 (6.19.3) เป็น v7 (7.8.0):**
    - ปรับปรุงการเชื่อมต่อฐานข้อมูลให้เป็นระบบโมดูลาร์ตามมาตรฐาน Prisma 7 โดยการลบ `url` ออกจาก `schema.prisma` และย้ายไปตั้งค่าใน `apps/api/prisma.config.ts` แทน
    - นำระบบ **Driver Adapter** มาใช้งานร่วมกับ `@prisma/adapter-pg` และ `pg` เพื่อเพิ่มความเสถียรและประสิทธิภาพของ client connection
    - ลบโฟลเดอร์ `prisma` เก่าที่ซ้ำซ้อนในระดับ Root ของโครงการทิ้งเพื่อความเป็นระเบียบเรียบร้อย
    - ปรับปรุงระบบฐานข้อมูลจำลอง (Seed Data) โดยย้ายคำสั่ง Seed ไปกำหนดใน `prisma.config.ts` และปรับปรุง `seed.ts` ให้ใช้ Driver Adapter แทนการใช้งาน client แบบเดิม
